...
return new class extends Migration
{
    ...
    {
        Schema::create('estimate_vouchers', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->date('date')->index(); // 発行日
            $table->date('expiry_date')->index(); // 有効期限日
            $table->text('customer_note')->nullable(); // 顧客向け備考欄
    ...
    {
        Schema::dropIfExists('estimate_vouchers');
    }
};
