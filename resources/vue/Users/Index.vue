<template>
    <div class="container">
        <div class="row g-4">
            ...
            <div class="col-4">
                <router-link :to="{ name: 'users.store' }" class="btn btn-primary w-100 rounded-0">新規追加</router-link>
            </div>
                    ...
                    <tbody>
                        <tr v-for="row in data" :key="row.id">
                            ...
                            <td class="p-0 text-center" style="vertical-align: middle;">
                                <router-link 
                                    :to="{ name: 'users.update', params: { id: row.id } }"
                                    class="btn btn-sm rounded-0"
                                    title="編集"
                                ><i class="fa-regular fa-pen-to-square"></i></router-link>

                                <router-link 
                                    :to="{ name: 'users.update_password', params: { id: row.id } }"
                                    class="btn btn-sm rounded-0"
                                    title="パスワード更新"
                                ><i class="fa-solid fa-key"></i></router-link>
                            </td>
                        </tr>
                    </tbody>
                </table>
...
<script setup lang="ts">
    ...
    const fetchIndex = async () => {
        ...
        await api.get(`/users${stringify(filter.value, page.value)}`)
        .then((res) => {
