<template>
    <div class="container">
        <div class="row g-4">
            ...
            <div class="col-4">
                <router-link :to="{ name: 'estimate_vouchers.store' }" class="btn btn-primary w-100 rounded-0">新規追加</router-link>
            </div>
                    ...
                    <tbody>
                        <tr v-for="row in data" :key="row.id">
                            ...
                            <td class="p-0 text-center" style="vertical-align: middle;">
                                <router-link 
                                    :to="{ name: 'estimate_vouchers.update', params: { id: row.id } }"
                                    class="btn btn-sm rounded-0"
                                    title="編集"
                                ><i class="fa-regular fa-pen-to-square"></i></router-link>

                                <Button class="btn-sm border-0 outline-none" @click="PdfModalRef.show(`/pdf/estimate-vouchers/${row.id}`);"><i class="fa-solid fa-print"></i></Button>
                            </td>
                        </tr>
                    </tbody>
                </table>
...
<script setup lang="ts">
    ...
    const fetchIndex = async () => {
        ...
        await api.get(`/estimate-vouchers${stringify(filter.value, page.value)}`)
        .then((res) => {
