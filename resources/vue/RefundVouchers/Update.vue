...
<script setup lang="ts">
    ...
    const show = async () => {
        ...
        try{
            let res = await api.get(`/refund-vouchers/${route.params.id}`);
    ...
    const update = async () => {
        ...
        try{
            let res = await api.put(`/refund-vouchers/${route.params.id}`, formatted.value);
    ...
    const destroy = async () => {
        ...
        try{
            let res = await api.delete(`/refund-vouchers/${route.params.id}`);

            alert('伝票を削除しました');

            router.push({ name: 'refund_vouchers.index' });
        }catch(err){
            // エラー処理は後で書きたい
