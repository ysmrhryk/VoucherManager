            ...
            <div class="row g-4">
                <div class="col-12">
                    <div class="input-group">
                        ...
                        <Input type="date" v-model="voucher.date" />
                    </div>
                </div>
                <div class="col-md-8">
                    <div class="input-group">
                        <span class="input-group-text rounded-0">担当者*</span>
                        <Input v-model.lazy="voucher.user.code" @change="getUser({ code: voucher.user.code })" placeholder="担当者コードを入力" />
            ...
            <div class="row g-4">
                <div class="col-md-12">
                    <div class="input-group">
                        ...
                        <Input v-model.lazy="voucher.client.code" @change="getClient({ code: voucher.client.code })" />
                        ...
                        <tr>
                            <td>
                                <select 
                                    ...
                                    v-model.lazy="line.service_voucher_row_type_id"
                                    @change="recalculation(index)"
                                >
                                    <option 
                                        v-for="service_voucher_row_type in service_voucher_row_types"
                                        :key="service_voucher_row_type.id"
                                        :value="service_voucher_row_type.id"
                                    >
                                        {{ service_voucher_row_type.value }}
                                    </option>
                                </select>
                            </td>
                            <td>
                                <Input 
                                    ...
                                    :disabled="![1,2,3,4].includes(line.service_voucher_row_type_id)"
                                    @change="recalculation(index)"
                                    placeholder="-"
                                />
                            </td>
                            <td>
                                <select 
                                    ...
                                    :disabled="![1,2,3].includes(line.service_voucher_row_type_id)"
                                    @change="recalculation(index)"
                                >
                                    <option :value="null">-</option>
                                    <option
                            ...
                            <td>
                                <Input 
                                    type="number" 
                                    class="border-0 text-end no-spin-btn" 
...
<script setup lang="ts">
    ...
    import type { NameSuffix, TaxRate, ServiceVoucherRowType } from '@/interfaces/Index';
    ...
    onMounted(async () => {
        [
            name_suffixes.value,
            service_voucher_row_types.value,
            tax_rates.value
        ] = await Promise.all([
            Request.name_suffixes(),
            Request.service_voucher_row_types(),
            Request.tax_rates()
        ]);
    ...
    const service_voucher_row_types = ref<ServiceVoucherRowType[]>();
    ...
    interface Line {
        localUuid: string,
        id: number | null,
        service_voucher_row_type_id: number,
    ...
    const show = async () => {
        ...
        try{
            let res = await api.get(`/service-vouchers/${route.params.id}`);
    ...
    const update = async () => {
        ...
        try{
            let res = await api.put(`/service-vouchers/${route.params.id}`, formatted.value);
    ...
    const destroy = async () => {
        ...
        try{
            let res = await api.delete(`/service-vouchers/${route.params.id}`);

            alert('伝票を削除しました');

            router.push({ name: 'service_vouchers.index' });
        }catch(err){
            // エラー処理は後で書きたい
    ...
    const recalculation = (index?: number) => {
        if(index !== undefined){
            ...
            if(line !== undefined){
                switch(line.service_voucher_row_type_id){
                    case 1: // 売上
    ...
    const addLine = (
        ...
        voucher.value.bodies.push({
            ...
            service_voucher_row_type_id: service_voucher_row_type_id || 1,
