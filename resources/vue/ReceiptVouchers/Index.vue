<template>
    <div class="container">
        <div class="row g-4">
            ...
            <div class="col-4">
                <router-link :to="{ name: 'receipt_vouchers.store' }" class="btn btn-primary w-100 rounded-0">新規追加</router-link>
            </div>
                        ...
                        <tr v-for="row in data" :key="row.id">
                            ...
                            <td>
                                <div class="d-flex">
                                    ...
                                    <div class="pe-1">{{ row.client_name }}</div>
                                    <div><small class="text-muted">{{ row.client.name_suffix.value }}</small></div>
                                </div>
                            </td>
                            <td class="text-end" :class="row.total_amount < 0 && 'text-danger'">{{ formatNumber(row.total_amount) }}</td>
                            <td>{{ formatDate(row.created_at) }}</td>
                            <td>{{ formatDate(row.updated_at) }}</td>
                            <td class="p-0 text-center" style="vertical-align: middle;">
                                <router-link 
                                    :to="{ name: 'receipt_vouchers.update', params: { id: row.id } }"
                                    class="btn btn-sm rounded-0"
                                    title="編集"
                                ><i class="fa-regular fa-pen-to-square"></i></router-link>

                                <a 
                                    :href="`/pdf/receipt-vouchers/${row.id}`" 
                                    class="btn btn-sm rounded-0"
...
<script setup lang="ts">
    ...
    const fetchIndex = async () => {
        ...
        await api.get(`/receipt-vouchers${stringify(filter.value, page.value)}`)
        .then((res) => {
