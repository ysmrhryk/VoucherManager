...
class ServiceVoucherController extends Controller
{
    ...
    {
        $data = QueryBuilder::for(ServiceVoucher::class)
        ->allowedFilters([
            AllowedFilter::custom('keywords', new ServiceVoucherKeywordSearch()), // キーワード検索
        ...
        ->with('user', 'client', 'name_suffix', 'client.name_suffix')
        ->allowedSorts(['date', 'created_at', 'updated_at'])
     ...
     */
    public function store(ServiceVoucherRequest $request)
    {
        $service_voucher = DB::transaction(function () use ($request) {
        ...
        return response()->json($service_voucher, 201); // 201 作成したデータを返す
    }

    /**
     * Display the specified resource.
     */
    public function show(ServiceVoucher $service_voucher)
    {
        return response()->json($service_voucher->fresh([
            'bodies', 
            'footers', 
            'user', 
            'name_suffix', 
     ...
     */
    public function update(ServiceVoucherRequest $request, ServiceVoucher $service_voucher)
    {
        DB::transaction(function () use ($request, $service_voucher) {
            $service_voucher = ServiceVoucher::lockForUpdate()->findOrFail($service_voucher->id);
            ...
            // ない行を削除
            $service_voucher->bodies()->whereNotIn('id', $bodies->pluck('id')->filter())->delete();
            $service_voucher->footers()->whereNotIn('tax_rate_id', $footers->pluck('tax_rate_id')->all())->delete();

            // 更新 or 登録
            $bodies->each(fn ($body) => $service_voucher->bodies()->updateOrCreate(['id' => $body['id'] ?? null], $body));
            $footers->each(fn ($footer) => $service_voucher->footers()->updateOrCreate(['tax_rate_id' => $footer['tax_rate_id']], $footer));
        });

        return response()->json($service_voucher->fresh()); // 200 編集した新しいデータを返す（データベースの今の状態を返す
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(ServiceVoucher $service_voucher)
    {
        $service_voucher->delete();

        return response()->noContent(); // 204
        // もしかしたら削除したデータに関して何か返したほうが、フロントが楽になるかもしれない
    }

    public function pdf(ServiceVoucher $service_voucher)
    {
        // データ加工
        $voucher_bodies = $service_voucher->bodies->map(function ($body) {
            $content = null;
            $tax_rate_name = null;
            $quantity = null;
            $unit_price = null;
            ...
            if(!empty($body)){
                ...
                if(in_array($body->service_voucher_row_type_id, [1,2,3], true)){
            ...
            return [
                'type' => 'body',
                'service_voucher_row_type_id' => $body->service_voucher_row_type_id,
                'content' => $content,
        ...
        $voucher_footers = $service_voucher->footers->map(function ($footer) {
            $tax_rate_name = null;
        ...
        $voucher_contents = [
            ...
            [
                'type' => 'footer',
                'title' => '合計',
                'total_net_amount' => number_format((int) $service_voucher->total_net_amount)
            ]
        ];

        $client_name = [null, null];

        if(!$service_voucher->extension_client_name){
            $client_name[0] = "{$service_voucher->client_name} {$service_voucher->client->name_suffix->value}";
        }else{
            $client_name[0] = "{$service_voucher->client_name}";
            $client_name[1] = "{$service_voucher->extension_client_name} {$service_voucher->name_suffix->value}";
        }

        $client_postal = null;

        if($service_voucher->client_postal){
            $client_postal = '〒' . $service_voucher->client_postal;
        }

        $my_postal = null;

        if($service_voucher->my_postal){
            $my_postal = '〒' . $service_voucher->my_postal;
        }
        ...
        for($p=0; $p<$allPages; $p++){
            ...
            $pdf->Cell($w*20, 0, Carbon::parse($service_voucher->date)->format('Y年m月d日'), 0, 1, 'R');
            ...
            $pdf->Cell($w*50, 5, $service_voucher->client_address, 0, 1, '', false, '', 1);

            // 得意先電話番号
            $pdf->setY(57);

            if($service_voucher->client_tel){
                $pdf->Cell($w*10, 5, 'TEL', 1, 0, 'C');
                $pdf->Cell($w*20, 5, $service_voucher->client_tel, 0, 0, 'C', false, '', 1);
                $pdf->Cell($w*20, 5, null, 0, 1, 'C', false, '', 1); // 余白分
            }

            if($service_voucher->client_fax){
                $pdf->Cell($w*10, 5, 'FAX', 1, 0, 'C');
                $pdf->Cell($w*20, 5, $service_voucher->client_fax, 0, 0, 'C', false, '', 1);
                $pdf->Cell($w*20, 5, null, 0, 1, 'C', false, '', 1); // 余白分
            }
            ...
            $pdf->Cell($w*35, 13, '¥ ' . number_format((int) $service_voucher->total_net_amount) . ' -', 1, 1, 'C', false, '', 1);
            ...
            $pdf->Cell($w*40, 5, $service_voucher->my_name, 0, 1, 'L', false, '', 1);
            ...
            $pdf->Cell($w*40, 5, $service_voucher->my_address, 0, 2, '', false, '', 1);

            $pdf->setY(57);

            if($service_voucher->my_tel){
                $pdf->Cell($w*10, 5, 'TEL', 1, 0, 'C');
                $pdf->Cell($w*20, 5, $service_voucher->my_tel, 0, 0, 'C', false, '', 1);
                $pdf->Cell($w*20, 5, null, 0, 1, 'C', false, '', 1); // 余白分
            }

            if($service_voucher->my_fax){
                $pdf->Cell($w*10, 5, 'FAX', 1, 0, 'C');
                $pdf->Cell($w*20, 5, $service_voucher->my_fax, 0, 0, 'C', false, '', 1);
                $pdf->Cell($w*20, 5, null, 0, 1, 'C', false, '', 1); // 余白分
            }
            ...
            for($i=0; $i<$perPage; $i++){
                ...
                if(!empty($line)){
                    switch(Arr::get($line, 'type', null)){
                        case 'body':
                            if(Arr::get($line, 'service_voucher_row_type_id', null) === 4){
            ...
            $pdf->multiCell($w*100, (($h*100) - $pdf->getY() + 10), $service_voucher->customer_note, 0, 'L', false, 1, '', '', true, 0, false, true, $h - ($pdf->getY() + 10), 'T', true);
            $pdf->setCellPaddings(1, 0, 1, 0);
            // 備考欄などここまで
        }

        $pdf->Output("{$service_voucher->id}.pdf", 'I');
    }
